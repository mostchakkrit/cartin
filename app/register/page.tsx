...
export default function RegisterPage() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-64px)]">
      ...
      {/* Right — form */}
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white dark:bg-[#111827] dark:bg-[#131c30] overflow-y-auto">
        <div className="w-full max-w-sm">
          <div className="lg:hidden mb-8 flex justify-center">
            <Link href="/"><Logo variant="dark" /></Link>
          </div>

          <p className="text-xs text-blue-600 font-semibold tracking-[0.2em] uppercase mb-2">Join Us</p>
          <h1 className="text-3xl font-bold text-slate-900 dark:text-[#dde8ff] mb-1">สมัครสมาชิก</h1>
          <p className="text-gray-400 dark:text-[#4e6888] text-sm mb-8">สร้างบัญชีใหม่ของคุณ</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-[#b8cef0] mb-1.5">ชื่อ-นามสกุล</label>
              <input
                type="text"
                value={form.name}
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <button type="button" onClick={() => setShowPw(!showPw)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 dark:text-[#4e6888] hover:text-gray-600 dark:text-[#8aaad4] p-1">
                  {showPw ? <EyeSlashIcon className="w-4 h-4" /> : <EyeIcon className="w-4 h-4" />}
                </button>
              </div>
              {form.password && (
                <p className={`text-xs mt-1.5 flex items-center gap-1 ${pwStrong ? "text-green-600" : "text-orange-500"}`}>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-[#b8cef0] mb-1.5">ยืนยันรหัสผ่าน</label>
              <div className="relative">
                <input
                  type="password"
                  value={form.confirm}
                  ...
                  required
                  className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 text-sm bg-gray-50 dark:bg-[#111827] focus:bg-white dark:bg-[#131c30] transition-colors ${
                    form.confirm ? (pwMatch ? "border-green-400 focus:ring-green-400" : "border-red-300 focus:ring-red-400") : "border-gray-200 dark:border-[#304070] focus:ring-blue-500"
                  }`}
                  placeholder="••••••••"
                />
                {pwMatch && (
